...
```bash
# 默认配置：6年训练 + 2年验证 + 1年测试
C:\Users\Administrator\miniconda3\envs\mystock\python.exe scripts\50_滚动窗口验证.py --config configs/workflow_config_top50_optimized.yaml
```
...
```bash
# 修改时间窗口大小
C:\Users\Administrator\miniconda3\envs\mystock\python.exe scripts\50_滚动窗口验证.py \
