...
def make_net_value_figure(nav, metrics, start_date, end_date):
    ...
    fig.update_layout(
        ...
        title=dict(text=f"v6 BBI月度强弱轮动  回测区间：{start_date} ~ {end_date}", x=0.02, xanchor="left"),
...
<div class="header">
  ...
  <p>候选过滤：价格达到近 21 日最高价 95% 以上不买；近 20 个交易日出现过收盘跌停不买；游资风险命中 2 项及以上不买；早期弱势下降趋势不买（前复权收盘价低于 20 日均线、20 日均线 10 日斜率为负、21 日收益为负）。</p>
</div>
{sections_html}
</body>
</html>"""
