...
实现一套长期持有策略：沿用现有 BBI 强势排名，等待候选股从阶段高点回撤后买入，按 `8 万 + 6 万 + 4 万 + 2 万` 分批建仓，最多持有 5 只，总投入最多 50 万，单票最多 20 万，并加入 -5% 止损、跌停硬止损、盈利持仓放量大阴线次日卖出。
...
- 保留现有 `score_candidates()`，强势排名沿用当前策略。
- 交易频率改为每日：
  - `T` 日收盘计算信号。
  - `T+1` 日开盘执行。
- 买入逻辑：
...
- 语法检查：
  - `python -m py_compile scripts/bbi/backtrader/v5/10_prepare_data.py scripts/bbi/backtrader/v5/20_run_backtest.py scripts/bbi/backtrader/v5/30_generate_report.py`
- 不主动运行完整回测，除非明确要求。
- 检查是否存在未来函数：
