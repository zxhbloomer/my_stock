# BBI v5i — 动态ATR止盈倍数（在v5e基础上）
# 核心改动：ATR止盈倍数根据ADX趋势强度动态调整
#   ADX >= 30（强趋势）：倍数 = 6.0x，让利润充分奔跑
#   ADX 15-30（中等趋势）：倍数 = 4.5x（v5e默认值）
#   ADX < 15（震荡/无趋势）：倍数 = 3.0x，收紧止盈保护利润
#
# 原理：趋势强时放宽止盈避免过早离场；震荡时收紧止盈锁定利润
# v5e baseline: calmar≈0.196, annual≈5.83%
import csv
...
ADX_PERIOD          = 14
ATR_MULT_STRONG     = 6.0   # ADX >= 30：强趋势，放宽止盈
ATR_MULT_NORMAL     = 4.5   # ADX 15-30：正常
ATR_MULT_WEAK       = 3.0   # ADX < 15：震荡，收紧止盈
ADX_STRONG_THRESH   = 30
ADX_WEAK_THRESH     = 15
TMP_OUTPUT_DIR      = Path(__file__).parent / "output"
V5_STOCK_DATA_DIR   = TMP_OUTPUT_DIR / "stock_data_fast"
OUTPUT_SUBDIR       = TMP_OUTPUT_DIR / "v5i"
...
class BBIV5iStrategy(bt.Strategy):
    def __init__(self):
    ...
    def _update_trail(self):
        ...
        if atr_val and atr_val > 0:
            mult = self._get_atr_mult()
            ns = self.peak_close - mult * atr_val
...
def run_single_stock(args):
    ...
    try:
        ...
        cerebro = bt.Cerebro()
        cerebro.addstrategy(BBIV5iStrategy)
...
def main():
    ...
    print('[v5i] {} stocks | dynamic ATR: weak={}x normal={}x strong={}x | ADX thresholds: {}/{}'.format(
        len(args_list), ATR_MULT_WEAK, ATR_MULT_NORMAL, ATR_MULT_STRONG,
        ADX_WEAK_THRESH, ADX_STRONG_THRESH))
