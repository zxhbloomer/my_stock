...
OUTPUT_DIR = Path(__file__).resolve().parent / "v4_strong_exit_output"
...
def is_strong_trend_leader(code, signal_panel, max_rank=5):
    if code not in signal_panel.index:
        return False
    candidates = score_candidates(signal_panel).reset_index(drop=True)
    if candidates.empty:
        return False
    matched = candidates[candidates["ts_code"] == code]
    if matched.empty:
        return False
    rank = int(matched.index[0]) + 1
    return bool(matched.iloc[0].get("strong_trend", False)) and rank <= max_rank
        ...
        if i > 0 and holdings:
            ...
            for code in list(holdings):
                ...
                if has_stop_loss_signal(code, pos, signal_panel):
                    ...
                    if is_strong_trend_leader(code, signal_panel):
                        strikes = int(pos.get("bearish_volume_strikes", 0)) + 1
                        pos["bearish_volume_strikes"] = strikes
