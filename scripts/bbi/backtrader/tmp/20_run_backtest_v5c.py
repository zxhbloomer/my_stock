# BBI v5c — v5a + tighter hard stop (6% instead of 8%)
# Hypothesis: v5a's hard stop rate jumped to 35.5% (from 22.9% in v2.4)
# because MIN_HOLD=20d blocks soft exits, letting losers ride to -8%
# Fix: tighten hard stop to 6% to cap single-trade loss
...
ATR_MULTIPLIER_V5C = 4.5
MIN_HOLD_DAYS_V5C  = 20
BBI_DECLINE_BARS   = 5
HARD_STOP_V5C      = 0.06   # tightened from 8% to 6%
OUTPUT_SUBDIR      = OUTPUT_DIR / "v5c"
...
class BBIV5cStrategy(bt.Strategy):
    def __init__(self):
    ...
    def _update_trail(self):
        ...
        if atr_val and atr_val > 0:
            new_stop = self.peak_close - ATR_MULTIPLIER_V5C * atr_val
    ...
    def _should_exit(self):
        ...
        if pos.size > 0:
            if (self.close_line[0] - pos.price) / pos.price <= -HARD_STOP_V5C:
                return True
        if len(self) - self.buy_bar < MIN_HOLD_DAYS_V5C:
            return False
...
def run_single_stock(args):
    ...
    try:
        ...
        df = df.sort_index()
        cerebro = bt.Cerebro()
        cerebro.addstrategy(BBIV5cStrategy)
...
def main():
    ...
    print(f'[v5c] {len(args_list)} stocks | ATR={ATR_MULTIPLIER_V5C}x | MIN_HOLD={MIN_HOLD_DAYS_V5C}d | HARD_STOP={HARD_STOP_V5C*100:.0f}%')
