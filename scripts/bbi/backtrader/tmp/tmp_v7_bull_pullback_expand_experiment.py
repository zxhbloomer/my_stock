...
OUTPUT_DIR = TMP_DIR / "tmp_v7_bull_pullback_expand_output"
REPORT_PATH = OUTPUT_DIR / "report.html"
README_PATH = TMP_DIR / "tmp_v7_bull_pullback_expand_README.md"
...
CASES = [
    ...
    {
        "case": "strong10回踩扩展_宽止损8",
        "mode": "pullback_expand_wide_stop",
        "bull_max_holdings": 7,
        "strong_bull_max_holdings": 10,
        "bull_total_exposure": 500_000.0,
        "strong_total_exposure": 550_000.0,
...
def is_pullback_ok(row, case: dict) -> bool:
    ...
    eps = 1e-12
    return (
        bbi_distance <= float(case.get("max_bbi_distance", 0.05)) + eps
...
def stop_loss_for_position(market_regime_name: str, regime_snapshot: dict, pos: dict, case: dict) -> float:
    if (
        (case.get("mode") == "pullback_expand_wide_stop" or "wide_stop_loss" in case)
        and is_strong_bull(market_regime_name, regime_snapshot, case)
        and bool(pos.get("pullback_entry", False))
    ):
...
def load_v7_backtest_module(module_name: str, case: dict):
    ...
    try:
        ...
        source = replace_once(
            ...
            '        "weak_lowvol_mom_candidate_blocks": 0,\n'
            '        "pullback_expand_case": PULLBACK_EXPAND_CASE.get("case", "baseline"),\n'
            '        "pullback_extension_buys": 0,\n'
            '        "pullback_add_buys": 0,\n',
        )
        source = replace_once(
            source,
        ...
        source = replace_once(
            ...
            "                        if pos.get(\"pending_sell\") or not __can_add_pullback(code, pos, signal_panel, market_regime_name, regime_snapshot):\n",
        )
        source = replace_once(
            source,
        ...
        source = replace_once(
            ...
            '                            stats["add_buy_fills"] += 1\n'
            '                            if __is_pullback_code(code, signal_panel):\n'
            '                                stats["pullback_add_buys"] += 1\n',
        )
        source = replace_once(
            source,
        ...
        module.PULLBACK_EXPAND_CASE = dict(case)

        def _is_pullback_code(code, signal_panel):
            if module.PULLBACK_EXPAND_CASE.get("mode") == "baseline":
                return False
            if code not in signal_panel.index:
                return False
            return is_pullback_ok(signal_panel.loc[code], module.PULLBACK_EXPAND_CASE)

        def _max_holdings(market_regime_name, regime_snapshot):
            return max_holdings_for_regime(market_regime_name, regime_snapshot, module.PULLBACK_EXPAND_CASE)

        def _total_limit(market_regime_name, regime_snapshot):
            return total_exposure_limit(market_regime_name, regime_snapshot, module.PULLBACK_EXPAND_CASE)

        def _is_extension_entry(holding_count_after_buy, market_regime_name, regime_snapshot):
            if module.PULLBACK_EXPAND_CASE.get("mode") == "baseline" or market_regime_name != "bull":
                return False
        ...
        def _initial_amount(current_holding_count, code, signal_panel, market_regime_name, regime_snapshot, available_exposure):
            if module.PULLBACK_EXPAND_CASE.get("mode") == "baseline" or current_holding_count < BASE_LONG_MAX_HOLDINGS:
                return min(float(module.LONG_POSITION_STEPS[0]), available_exposure)
            if market_regime_name != "bull" or not _is_pullback_code(code, signal_panel):
                return 0.0
            return min(float(module.PULLBACK_EXPAND_CASE.get("extension_initial_amount", 40_000.0)), available_exposure)
        ...
        def _can_add(code, pos, signal_panel, market_regime_name, regime_snapshot):
            ...
            if module.PULLBACK_EXPAND_CASE.get("mode") == "baseline":
                return True
            return _is_pullback_code(code, signal_panel)

        def _stop_trigger(market_regime_name, regime_snapshot, pos, profit_pct):
            threshold = stop_loss_for_position(market_regime_name, regime_snapshot, pos, module.PULLBACK_EXPAND_CASE)
            return profit_pct <= threshold

        module.__is_pullback_code = _is_pullback_code
        ...
        module.__can_add_pullback = _can_add
...
def run_case(case: dict, panel: pd.DataFrame, market: pd.DataFrame) -> tuple[dict, pd.DataFrame, pd.DataFrame]:
    module = load_v7_backtest_module("tmp_v7_bull_pullback_expand_" + case["case"], case)
...
def table_html(df: pd.DataFrame, title: str) -> str:
    ...
    rows = ["<tr>" + "".join(f"<td>{html.escape(str(v))}</td>" for v in row.tolist()) + "</tr>" for _, row in df.iterrows()]
...
def build_report(all_stats: list[dict], nav_by_case: dict[str, pd.DataFrame]) -> None:
    ...
    for stats in all_stats:
        summary_rows.append({
            ...
            "扩展买入": stats.get("pullback_extension_buys", "-"),
            "回踩加仓": stats.get("pullback_add_buys", "-"),
    ...
    yearly = {name: yearly_returns(nav) * 100.0 for name, nav in nav_by_case.items()}
    monthly = {name: monthly_returns(nav) * 100.0 for name, nav in nav_by_case.items()}
...
<p class="small">实验只修改 tmp 注入逻辑，正式 v4/v5/v6/v7 文件未修改。扩展仓和加仓只使用 signal_date 已知的回踩字段。</p>
{table_html(summary_df, "核心指标对比")}
{table_html(yearly_df, "年度收益对比（%）")}
{table_html(monthly_df, "最近36个月月度收益对比（%）")}
<h2>下一步建议</h2>
<ol>
<li>若收益不如 v7，不继续扩仓，转向失败买入过滤。</li>
<li>若收益超过 v7，再单独验证宽止损和总仓上限。</li>
<li>如果扩展仓交易很少，说明回踩条件过严，应先分析触发率，不直接放宽仓位。</li>
</ol>
</body>
</html>"""
...
def main() -> None:
    ...
    append_progress("设计 review：采纳专家建议，第一版只做回踩扩展仓，不新增财务/行业/资金流。")
    append_progress("开发 review：正式 v4-v7 文件未修改；规则函数有单元测试；输出写入 tmp。")
