# v4k_backtest.py
# v4j 修复版：加涨跌停板流动性过滤
# 买入时：如果当天是一字涨停板（open==close 且涨幅>=9.5%），跳过，无法成交
# 卖出时：如果当天是一字跌停板（open==close 且跌幅<=-9.5%），用跌停价成交（已是最差价）
# 其余逻辑与 v4j 完全相同
import sys
...
def build_panel(data):
    ...
    for code, df in data.items():
        tmp = df[['trade_date', 'open_qfq', 'close_qfq', 'bbi_qfq', 'prev_close']].copy()
        tmp['ts_code'] = code
        frames.append(tmp)
...
def is_limit_up(day_panel, code):
    """当天是否是一字涨停板（无法买入）"""
    if code not in day_panel.index:
        return False
    row = day_panel.loc[code]
    cur_open  = float(row['open_qfq'])
    cur_close = float(row['close_qfq'])
    prev_close = float(row['prev_close']) if pd.notna(row['prev_close']) else cur_close
    if prev_close <= 0:
        return False
    pct = (cur_close - prev_close) / prev_close
    # 一字板：开盘价等于收盘价，且涨幅达到涨停线
    return abs(cur_open - cur_close) < cur_close * 0.001 and pct >= LIMIT_UP_THRESHOLD


def is_limit_down(day_panel, code):
    """当天是否是一字跌停板（无法卖出，只能按跌停价记录）"""
    if code not in day_panel.index:
        return False
    row = day_panel.loc[code]
    cur_open  = float(row['open_qfq'])
    ...
    if prev_close <= 0:
        return False
    pct = (cur_close - prev_close) / prev_close
    return abs(cur_open - cur_close) < cur_close * 0.001 and pct <= LIMIT_DOWN_THRESHOLD
...
def run_backtest(data, data_indexed, panel, name_map, index_df):
    ...
    print(f'跳过一字涨停板买入: {skipped_limit_up} 次')
