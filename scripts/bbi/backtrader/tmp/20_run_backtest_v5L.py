...
OUTPUT_SUBDIR       = TMP_OUTPUT_DIR / "v5L"
...
class BBIV5LStrategy(bt.Strategy):
    def __init__(self):
...
def run_single_stock(args):
    ...
    try:
        ...
        cerebro = bt.Cerebro()
        cerebro.addstrategy(BBIV5LStrategy)
...
def main():
    ...
    print('[v5L] {} stocks | vol_high={}x({}%) vol_low={}x({}%) skip<{}x'.format(
        len(args_list), VOL_HIGH_MULT, int(RATIO_HIGH*100),
        VOL_LOW_MULT, int(RATIO_LOW*100), VOL_LOW_MULT))
