...
OUTPUT_DIR = TMP_DIR / "tmp_v6_2018_bear_dividend_event_evolution_output"
REPORT_PATH = OUTPUT_DIR / "report.html"
RESULTS_PATH = OUTPUT_DIR / "results.csv"
README_PATH = TMP_DIR / "tmp_v6_2018_bear_dividend_event_evolution_README.md"
...
CASES = [
    {"name": "event_ex_date_strict_30_monthly", "max_holdings": 30, "max_per_industry": 3, "industry_neutral": True, "quality_trap": True, "dividend_mode": "strict"},
    {"name": "event_ann_date_known_30_monthly", "max_holdings": 30, "max_per_industry": 3, "industry_neutral": True, "quality_trap": True, "dividend_mode": "known"},
    {"name": "event_strict_pending_30_monthly", "max_holdings": 30, "max_per_industry": 3, "industry_neutral": True, "quality_trap": True, "dividend_mode": "strict_pending"},
]
...
def quality_trap_mask(frame: pd.DataFrame) -> pd.Series:
    if "pit_div_yield_strict" in frame.columns:
        dv = pd.to_numeric(frame["pit_div_yield_strict"], errors="coerce")
    else:
...
def load_support_data(db_url: str, schema: str, start_date: str, end_date: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    with engine.connect() as conn:
        ...
        dividend = pd.read_sql_query(
            text(f"""select ts_code, end_date, ann_date, record_date, ex_date, cash_div_tax, div_proc from {schema}."041_dividend" where ann_date <= :end_date"""),
            conn,
            params={"end_date": end_date},
        )
    return daily_basic, fina, stock_basic, dividend
...
def normalize_dividend_frame(dividend: pd.DataFrame) -> pd.DataFrame:
    out = dividend.copy()
    for col in ["end_date", "ann_date", "record_date", "ex_date"]:
        out[col] = pd.to_datetime(out[col], errors="coerce")
    out["cash_div_tax"] = pd.to_numeric(out["cash_div_tax"], errors="coerce").fillna(0.0)
    out = out.dropna(subset=["ts_code", "end_date", "ann_date"]).sort_values(["ts_code", "end_date", "ann_date"]).reset_index(drop=True)
    return out


def resolve_latest_dividend_state(dividend: pd.DataFrame, as_of_date: pd.Timestamp) -> pd.DataFrame:
    if dividend.empty:
        return dividend.copy()
    frame = normalize_dividend_frame(dividend)
    frame = frame[frame["ann_date"] <= pd.Timestamp(as_of_date)].copy()
    if frame.empty:
        return frame
    frame = frame.sort_values(["ts_code", "end_date", "ann_date"]).drop_duplicates(subset=["ts_code", "end_date"], keep="last")
    return frame.reset_index(drop=True)


def compute_event_dividend_features(dividend: pd.DataFrame, as_of_date: pd.Timestamp, price: float) -> dict[str, float]:
    as_of = pd.Timestamp(as_of_date)
    resolved = resolve_latest_dividend_state(dividend, as_of)
    if resolved.empty or not np.isfinite(price) or price <= 0:
        return {"pit_div_yield_strict": 0.0, "pit_div_yield_known": 0.0, "pending_div_yield": 0.0}
    window_start = as_of - pd.Timedelta(days=365)
    positive = resolved[resolved["cash_div_tax"] > 0].copy()
    known = positive[positive["ann_date"] >= window_start]
    strict = positive[
        positive["ex_date"].notna()
        & (positive["ex_date"] <= as_of)
        & (positive["ex_date"] >= window_start)
    ]
    pending = positive[
        (positive["ann_date"] <= as_of)
        & positive["record_date"].notna()
        & (positive["record_date"] > as_of)
        & ((positive["ex_date"].isna()) | (positive["ex_date"] > as_of))
    ]
    return {
        "pit_div_yield_strict": float(strict["cash_div_tax"].sum() / price),
        "pit_div_yield_known": float(known["cash_div_tax"].sum() / price),
        "pending_div_yield": float(pending["cash_div_tax"].sum() / price),
    }


def compute_event_dividend_features_for_code(stock_panel: pd.DataFrame, stock_dividend: pd.DataFrame) -> pd.DataFrame:
    dates = pd.to_datetime(stock_panel["trade_date"]).tolist()
    prices = pd.to_numeric(stock_panel["close"], errors="coerce").tolist()
    codes = stock_panel["ts_code"].tolist()
    if stock_dividend.empty:
        return pd.DataFrame(
            {
                "trade_date": dates,
                "ts_code": codes,
                "pit_div_yield_strict": 0.0,
                "pit_div_yield_known": 0.0,
                "pending_div_yield": 0.0,
            }
        )

    events = normalize_dividend_frame(stock_dividend).sort_values("ann_date").to_dict("records")
    latest_by_end_date: dict[pd.Timestamp, dict] = {}
    event_idx = 0
    rows = []
    for trade_date, code, price in zip(dates, codes, prices):
        while event_idx < len(events) and events[event_idx]["ann_date"] <= trade_date:
            event = events[event_idx]
            latest_by_end_date[event["end_date"]] = event
            event_idx += 1
        if not np.isfinite(price) or price <= 0 or not latest_by_end_date:
            rows.append({"trade_date": trade_date, "ts_code": code, "pit_div_yield_strict": 0.0, "pit_div_yield_known": 0.0, "pending_div_yield": 0.0})
            continue
        window_start = trade_date - pd.Timedelta(days=365)
        strict_sum = 0.0
        known_sum = 0.0
        pending_sum = 0.0
        for event in latest_by_end_date.values():
            cash = float(event.get("cash_div_tax", 0.0) or 0.0)
            if cash <= 0:
                continue
            ann_date = event.get("ann_date")
            ex_date = event.get("ex_date")
            record_date = event.get("record_date")
            if pd.notna(ann_date) and ann_date >= window_start:
                known_sum += cash
            if pd.notna(ex_date) and ex_date <= trade_date and ex_date >= window_start:
                strict_sum += cash
            if pd.notna(record_date) and record_date > trade_date and (pd.isna(ex_date) or ex_date > trade_date):
                pending_sum += cash
        rows.append(
            {
                "trade_date": trade_date,
                "ts_code": code,
                "pit_div_yield_strict": strict_sum / price,
                "pit_div_yield_known": known_sum / price,
                "pending_div_yield": pending_sum / price,
            }
        )
    return pd.DataFrame(rows)


def add_dividend_event_features(panel: pd.DataFrame, dividend: pd.DataFrame) -> pd.DataFrame:
    out = panel.copy()
    frame = normalize_dividend_frame(dividend)
    dividend_by_code = {code: group for code, group in frame.groupby("ts_code", sort=False)}
    features = []
    for code, group in out.groupby("ts_code", sort=False):
        features.append(compute_event_dividend_features_for_code(group[["trade_date", "ts_code", "close"]].sort_values("trade_date"), dividend_by_code.get(code, pd.DataFrame())))
    feature_df = pd.concat(features, ignore_index=True)
    return out.merge(feature_df, on=["trade_date", "ts_code"], how="left")


def build_base_panel(panel: pd.DataFrame, market: pd.DataFrame, daily_basic: pd.DataFrame, fina: pd.DataFrame, stock_basic: pd.DataFrame, dividend: pd.DataFrame) -> pd.DataFrame:
    ...
    enriched = add_dividend_event_features(enriched, dividend)
...
def compute_defensive_score(frame: pd.DataFrame, industry_neutral: bool, dividend_mode: str) -> pd.DataFrame:
    out = frame.copy()
    if industry_neutral:
    ...
    if dividend_mode == "strict":
        dividend_source = out["pit_div_yield_strict"]
        pending_tiebreak = 0.0
    elif dividend_mode == "known":
        dividend_source = out["pit_div_yield_known"]
        pending_tiebreak = 0.0
    elif dividend_mode == "strict_pending":
        dividend_source = out["pit_div_yield_strict"]
        pending_tiebreak = 0.03 * zscore(winsorize_series(out["pending_div_yield"]))
    else:
        dividend_source = out["dv_ttm"]
        pending_tiebreak = 0.0
    dividend = zscore(winsorize_series(dividend_source))
    quality = (
    ...
    out["defensive_score"] = 0.35 * (0.30 * risk1 + 0.30 * risk2 + 0.20 * risk3 + 0.20 * risk4) + 0.25 * dividend + 0.20 * quality + 0.10 * value + 0.10 * momentum + pending_tiebreak
    return out


def build_signal_panel(base_panel: pd.DataFrame, case: dict) -> pd.DataFrame:
    enriched = compute_defensive_score(base_panel, industry_neutral=case["industry_neutral"], dividend_mode=case.get("dividend_mode", "dv_ttm"))
    dividend_field = {"strict": "pit_div_yield_strict", "known": "pit_div_yield_known", "strict_pending": "pit_div_yield_strict"}.get(case.get("dividend_mode"), "dv_ttm")
    base = (
        ...
        & (pd.to_numeric(enriched[dividend_field], errors="coerce") > 0)
...
def run_backtest(signal_panel: pd.DataFrame, max_holdings: int, max_per_industry: int | None) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    ...
    for i, date in enumerate(dates):
        ...
        if month_start:
            ...
            if max_per_industry is None:
                selected = candidates.sort_values("defensive_score", ascending=False).head(max_holdings)
            else:
                selected = select_with_industry_cap(candidates, max_holdings=max_holdings, max_per_industry=max_per_industry)
            ...
            target_amount = cash / max(max_holdings - len(holdings), 1)
            for code in target_codes:
                ...
                trades.append({"date": str(date)[:10], "ts_code": code, "action": "buy", "price": price, "shares": shares, "reason": "defensive"})
        ...
        nav_rows.append({"date": str(date)[:10], "nav": nav, "cash": cash, "holdings": len(holdings), "industry_peak": industry_peak})
...
def render_report(compare: pd.DataFrame, case_results: pd.DataFrame, yearly: pd.DataFrame, monthly: pd.DataFrame, notes: str):
    ...
    text = f"""<!doctype html>
<html lang="zh-CN"><head><meta charset="utf-8"><title>2018 独立熊市分红事件进化</title>
<style>
...
<h1>2018 独立熊市分红事件进化</h1>
<div class="note">本轮只优化独立熊市策略，不接入 v6 空仓逻辑。红利项改为 041_dividend 的 PIT 事件口径，行业仍使用 stock_basic.industry。</div>
...
def main():
    ...
    README_PATH.write_text("# tmp_v6 2018 独立熊市分红事件进化进度\n\n", encoding="utf-8")
    ...
    daily_basic, fina, stock_basic, dividend = load_support_data(v6_config.DB_URL, v6_config.SCHEMA, str(panel["trade_date"].min().date()), str(panel["trade_date"].max().date()))
    append_progress("完成 daily_basic/fina_indicator/stock_basic/dividend 加载。")

    base_panel = build_base_panel(panel, market, daily_basic, fina, stock_basic, dividend)
    append_progress("完成含 dividend PIT 的基础面板构建。")
    ...
    notes = (
        "策略专家意见：主版本优先采用 ann_date 可见、ex_date 落地 的严格 PIT 分红口径，避免把预案直接当成已实现现金分红。\n"
        "代码/数据说明：行业使用 stock_basic.industry；申万成员表当前为空。041_dividend 同一 end_date 多次公告时，本实验按时点只保留最新已知状态。\n"
        ...
        f"建议：{merge_advice}。\n"
        "限制：2018 年 1 月仍可能有空仓偏差；停牌时仍沿用最近价格，偏乐观。\n"
        "下一步：如果最佳变体仍未优于 v6，则更值得优先修复起跑线一致性与空仓/仓位机制，而不是继续扩大分红参数搜索。"
    )
