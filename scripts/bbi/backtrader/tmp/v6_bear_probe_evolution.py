...
OUTPUT_DIR = TMP_DIR / "v6_bear_probe_evolution_output"
RESULTS_PATH = OUTPUT_DIR / "results.csv"
README_PATH = TMP_DIR / "v6_bear_probe_evolution_README.md"
DESIGN_PATH = TMP_DIR / "v6_bear_probe_evolution_design.md"
PLAN_PATH = TMP_DIR / "v6_bear_probe_evolution_plan.md"
REPORT_PATH = OUTPUT_DIR / "report.html"

SOURCE_NOTES = [
    {
        "topic": "市场宽度",
        "source": "QuantifiedStrategies market breadth",
        "url": "https://www.quantifiedstrategies.com/market-breadth",
        "note": "市场宽度用于确认指数走势内部参与度；单独交易信号不足，适合与趋势条件组合。",
    },
    {
        "topic": "宽度 thrust",
        "source": "TradingSim Breadth Thrust",
        "url": "https://www.tradingsim.com/blog/breadth-thrust-indicator",
        "note": "Zweig 宽度 thrust 关注从超卖到强势的快速修复，常用于识别熊转牛早期。",
    },
    {
        "topic": "熊市反弹",
        "source": "Wilshire bear market rallies",
        "url": "https://www.wilshire.com/blog/differentiating-bear-market-rallies-vs-meaningful-inflection-points",
        "note": "熊市中反弹频繁，需技术确认区分反弹和持续拐点。",
    },
    {
        "topic": "右侧交易",
        "source": "百度百科右侧交易法",
        "url": "https://baike.baidu.com/item/%E5%8F%B3%E4%BE%A7%E4%BA%A4%E6%98%93%E6%B3%95/4494346",
        "note": "右侧交易强调趋势明确后介入，熊市不盲目抄底。",
    },
]
...
CASES = [
    {
        "name": "baseline_v6",
        "probe_fraction": 0.0,
        "max_probe_exposure": 0.0,
        "min_breadth": 0.0,
        "breadth_improve": 0.0,
        "description": "当前 v6：熊市确认后不开仓。",
    },
    {
        "name": "bear_probe_15pct",
        "probe_fraction": 0.15,
        "max_probe_exposure": 75_000.0,
        "min_breadth": 0.35,
        "breadth_improve": 0.08,
        "description": "熊市宽度修复后，以正常首买 15% 小仓位试探。",
    },
    {
        "name": "bear_probe_25pct",
        "probe_fraction": 0.25,
        "max_probe_exposure": 100_000.0,
        "min_breadth": 0.35,
        "breadth_improve": 0.10,
        "description": "熊市宽度修复后，以正常首买 25% 小仓位试探，总试探仓不超过 10 万。",
    },
    {
        "name": "bear_probe_25pct_strict",
        "probe_fraction": 0.25,
        "max_probe_exposure": 100_000.0,
        "min_breadth": 0.45,
        "breadth_improve": 0.12,
        "description": "更严格宽度修复门槛，减少熊市假反弹买入。",
    },
]
...
def load_v6_module():
    return load_module_from_path("v6_run_backtest_bear_probe", V6_DIR / "20_run_backtest.py")
...
def add_market_probe_features(market_regime: pd.DataFrame, min_breadth=0.35, breadth_improve=0.10) -> pd.DataFrame:
...
def build_market_probe_table(v6, market, panel, case):
    regime = v6.build_market_regime(market, panel).reset_index()
    return add_market_probe_features(
        regime,
        min_breadth=case["min_breadth"],
        breadth_improve=case["breadth_improve"],
    )


def run_bear_probe_backtest(v6, panel, market, case, start_date, end_date):
    market_regime = v6.build_market_regime(market, panel)
    probe_market = build_market_probe_table(v6, market, panel, case)
    if end_date:
    ...
    for i, date in enumerate(all_dates):
        ...
        if i > 0:
            ...
            probe_ok = bool(probe_market.loc[signal_date, "bear_probe_market_ok"]) if signal_date in probe_market.index else False
            normal_open = not short_drop_blocked and not regime_blocked
            probe_open = regime_blocked and probe_ok and case["probe_fraction"] > 0
            ...
            if not normal_open and not probe_open:
                ...
                for code in target_codes:
                    ...
                    available_exposure = v6.LONG_MAX_TOTAL_EXPOSURE - v6.calc_total_exposure(holdings)
                    if probe_open:
                        probe_exposure = sum(float(pos.get("invested_amount", 0.0)) for pos in holdings.values() if pos.get("probe_entry"))
                        target_amount = calc_bear_probe_target_amount(
                            float(v6.LONG_POSITION_STEPS[0]),
                            cash,
                            probe_exposure,
                            case["max_probe_exposure"],
                            case["probe_fraction"],
                        )
                    else:
                    ...
                    if target_amount < 100:
                        break
                    reason = "bear_probe_initial_buy" if probe_open else "long_initial_buy"
...
def run_case(v6, panel, market, case):
    if case["name"] == "baseline_v6":
        ...
        nav, trades, rebalance, scores, holdings, stats = run_bear_probe_backtest(v6, panel, market, case, "2018-01-01", "2026-05-14")
    rows = []
...
def _result_row(case, period, metrics, stats, trades, start=None, end=None):
    ...
    probe_buys = int(stats.get("bear_probe_buys", 0))
    if start is not None and end is not None and not trades.empty:
        ...
        probe_buys = int((segment_trades["reason"] == "bear_probe_initial_buy").sum()) if "reason" in segment_trades.columns else 0
    return {
        "case": case["name"],
        "period": period,
...
def recommendation_for(best, baseline, walk, results=None):
    if results is not None and best["case"] != "baseline_v6":
        ...
        if (
            ...
            return "建议进入合并候选：全周期、验证期和确认期均优于当前 v6，回撤没有明显恶化；合并前继续做参数邻域和滑点压力测试。"
        return (
    ...
    return "暂不建议合并：本轮熊市试探买入没有同时提升收益和控制回撤，当前 v6 的熊市不开仓仍是更稳基线。"
...
def write_design_and_plan():
    design = """# v6 Bear Probe Evolution Design
...
"""
    plan = """# v6 Bear Probe Evolution Plan

1. 写测试：市场宽度修复、个股 BBI 修复、试探仓位上限。
...
def write_outputs(results, v4_summary, v5_summary, v6_summary, walk, nav_by_label):
    ...
    best = full.iloc[0]
    advice = recommendation_for(best, baseline, walk, results)
    labels = ["v4", "v5", "v6", str(best["case"])]
    ...
    for _, row in full.iterrows():
        ...
        full_rows.append(
            ...
            f"<td>{int(row['bear_probe_signal_days'])}</td><td>{int(row['bear_probe_buys'])}</td>"
    ...
    for _, row in full.iterrows():
        readme.append(f"| {row['case']} | {row['total_return_pct']:.2f}% | {row['annual_return_pct']:.2f}% | {row['max_drawdown_pct']:.2f}% | {row['calmar_ratio']:.4f} | {int(row['bear_probe_buys'])} |")
    readme.extend([
        "",
        "## 口径说明",
    ...
    page = f"""<!DOCTYPE html>
<html lang="zh-CN"><head><meta charset="UTF-8"><title>v6 熊市试探买入实验</title>
<style>
...
</style></head><body><header><h1>v6 熊市试探买入实验</h1><p>只在熊市右侧修复时小仓位试探，目标是提升收益，同时避免抄底式扩大回撤。</p></header><main>
...
<section class="advice"><h2>是否合并</h2><p><strong>{html_escape(advice)}</strong></p><p>Walk-forward 训练选择：{html_escape(walk['selected_case'])}；验证期收益 {walk['validate_total_return_pct']:.2f}%，确认期收益 {walk['confirm_total_return_pct']:.2f}%。</p><p>口径：baseline_v6 由本脚本复跑并断言匹配 v6 summary；v4/v5 读取既有 output，仅作背景比较。</p></section>
<section><h2>全周期候选对比</h2><table><thead><tr><th>case</th><th>总收益</th><th>年化</th><th>最大回撤</th><th>Calmar</th><th>交易数</th><th>试探信号日</th><th>试探买入</th><th>说明</th></tr></thead><tbody>{''.join(full_rows)}</tbody></table></section>
...
<section><h2>下一步</h2><ol><li>若候选超过 v6：继续做参数邻域和滑点压力测试。</li><li>若没有超过 v6：保留 v6 熊市不开仓，转向熊市退出优化。</li><li>不要把熊市低价直接等同买点。</li></ol></section>
</main></body></html>"""
...
def main():
    ...
    panel = add_stock_probe_features(panel)
