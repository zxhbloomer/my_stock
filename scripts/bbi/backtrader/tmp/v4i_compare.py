...
def main():
    print("=" * 72)
    print("  v4_plan_1 vs v4h（收紧止损）vs v4i（+市场仓位控制）对比报告")
    print("=" * 72)

    for p, name in [(V1_NAV, 'v4_plan_1'), (V4H_NAV, 'v4h'), (V4I_NAV, 'v4i')]:
        if not p.exists():
    ...
    m3, yr3 = calc_metrics(V4I_NAV, 'v4i')

    print("\n【核心指标对比】")
    keys = ['累积净值', '年化收益', '最大回撤', '夏普比率', '卡玛比率', '最终资产(元)']
    print(f"{'指标':<16} {'v4_plan_1':>16} {'v4h':>16} {'v4i':>16}")
    print("-" * 66)
    for k in keys:
    ...
    t3 = calc_trade_stats(V4I_TRADES, 'v4i')
    tkeys = ['总交易次数', '卖出次数', '止损次数', '胜率', '平均每笔盈亏(元)']
    print(f"{'指标':<20} {'v4_plan_1':>14} {'v4h':>14} {'v4i':>14}")
    print("-" * 64)
    for k in tkeys:
    ...
    print(f"{'年份':<8} {'v4_plan_1':>12} {'v4h':>12} {'v4i':>12} {'v4i-v1':>10}")
    print("-" * 58)
    for yr in all_years:
    ...
    print(f"结论: v4i 年化 {'↑' if v3_ann>v1_ann else '↓'}{abs(v3_ann-v1_ann):.2f}pp，"
          f"回撤 {'改善' if v3_dd>v1_dd else '恶化'}{abs(v3_dd-v1_dd):.2f}pp vs v4_plan_1")
    print("=" * 72)
