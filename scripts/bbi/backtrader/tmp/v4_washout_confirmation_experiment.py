...
WASHOUT_CONFIRM_MODE = "current"
TMP_OUTPUT_ROOT = Path(__file__).resolve().parent
OUTPUT_DIR = TMP_OUTPUT_ROOT / "v4_washout_current_output"
...
def parse_args():
    ...
    parser.add_argument(
        "--mode",
        default="current",
        choices=["current", "next_day", "three_day", "mid_next_day", "mid_three_day"],
        help="current=原始规则；next_day/three_day=宽确认；mid_next_day/mid_three_day=必须收复大阴线中位",
    )
...
def is_strong_trend_row(row):
    if row is None:
        return False
    if HOT_MONEY_RISK_ENABLED:
        hot_money_hits = safe_float(row.get("hot_money_risk_hits"))
        if pd.isna(hot_money_hits) or hot_money_hits >= HOT_MONEY_RISK_MIN_HITS:
            return False
    recent_limit_down = safe_float(row.get("recent_limit_down_20"))
    return (
        ...
        and not pd.isna(recent_limit_down)
        and recent_limit_down == 0
    )


def start_washout_confirmation(pos, signal_date, row):
    open_price = safe_float(row.get("open"))
    close_price = safe_float(row.get("close"))
    low_price = safe_float(row.get("low"))
    high_price = safe_float(row.get("high"))
    pos["washout_pending"] = True
    pos["washout_signal_date"] = str(signal_date)[:10]
    pos["washout_days"] = 0
    pos["washout_close"] = close_price
    pos["washout_low"] = low_price
    pos["washout_mid"] = (open_price + close_price) / 2.0
    pos["washout_range_mid"] = (high_price + low_price) / 2.0


def clear_washout_confirmation(pos):
    for key in [
        "washout_pending",
        "washout_signal_date",
        "washout_days",
...
def evaluate_washout_confirmation(code, pos, signal_panel):
    if code not in signal_panel.index:
        pos["washout_days"] = int(pos.get("washout_days", 0)) + 1
        return "hold"
    row = signal_panel.loc[code]
    apply_adj_factor(pos, row)
    close_price = safe_float(row.get("close"))
    ...
    pos["last_price"] = close_price
    washout_close = safe_float(pos.get("washout_close"))
    washout_low = safe_float(pos.get("washout_low"))
    washout_mid = safe_float(pos.get("washout_mid"))
    washout_range_mid = safe_float(pos.get("washout_range_mid"))
    if WASHOUT_CONFIRM_MODE.startswith("mid_"):
        recovered = close_price >= washout_mid or close_price >= washout_range_mid
    else:
        recovered = (
            close_price >= washout_mid
            or close_price >= washout_range_mid
            or close_price > washout_close
        )
    failed = close_price < washout_low or has_bearish_volume_signal(code, pos, signal_panel)
    days = int(pos.get("washout_days", 0)) + 1
    pos["washout_days"] = days

    if recovered:
        return "confirmed"
    if failed:
        return "failed"
    if WASHOUT_CONFIRM_MODE in {"next_day", "mid_next_day"}:
        return "failed"
    if WASHOUT_CONFIRM_MODE in {"three_day", "mid_three_day"} and days >= 3:
        return "failed"
    return "hold"
...
def run_backtest(panel, market, start_date, end_date):
    ...
    stats = {
        ...
        "washout_deferred_signals": 0,
        "washout_confirmed_keeps": 0,
        "washout_failed_exits": 0,
    }
        ...
        if i > 0 and holdings:
            ...
            for code in list(holdings):
                ...
                if pos.get("washout_pending"):
                    if has_stop_loss_signal(code, pos, signal_panel):
                        ...
                        washout_result = evaluate_washout_confirmation(code, pos, signal_panel)
                        if washout_result == "confirmed":
                            clear_washout_confirmation(pos)
                            stats["washout_confirmed_keeps"] += 1
                        elif washout_result == "failed":
                            clear_washout_confirmation(pos)
                            exit_reasons[code] = "long_bearish_volume_exit_confirmed_failed"
                            stats["bearish_volume_exit_signals"] += 1
                            stats["washout_failed_exits"] += 1
                    continue
                if has_stop_loss_signal(code, pos, signal_panel):
                    exit_reasons[code] = "long_stop_loss"
                    stats["stop_loss_signals"] += 1
                    ...
                    stats["bearish_volume_exit_signals"] += 1
                    row = signal_panel.loc[code] if code in signal_panel.index else None
                    if WASHOUT_CONFIRM_MODE != "current" and is_strong_trend_row(row):
                        start_washout_confirmation(pos, signal_date, row)
                        stats["washout_deferred_signals"] += 1
                    else:
            ...
            for code, exit_reason in exit_reasons.items():
                ...
                if sold:
                    ...
                    elif exit_reason.startswith("long_bearish_volume_exit"):
...
def main():
    global WASHOUT_CONFIRM_MODE
    global OUTPUT_DIR, NAV_SERIES_PATH, TRADE_RECORDS_PATH, REBALANCE_LOG_PATH
    global SCORES_PATH, SUMMARY_PATH, LAST_HOLDINGS_PATH

    args = parse_args()
    WASHOUT_CONFIRM_MODE = args.mode
    OUTPUT_DIR = TMP_OUTPUT_ROOT / f"v4_washout_{args.mode}_output"
    NAV_SERIES_PATH = OUTPUT_DIR / "nav_series.csv"
    TRADE_RECORDS_PATH = OUTPUT_DIR / "trade_records.csv"
    REBALANCE_LOG_PATH = OUTPUT_DIR / "rebalance_log.csv"
    SCORES_PATH = OUTPUT_DIR / "strength_scores.csv"
