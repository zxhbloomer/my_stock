...
OUTPUT_DIR = TMP_DIR / "tmp_v7_bear_hysteresis_gate_output"
REPORT_PATH = OUTPUT_DIR / "report.html"
README_PATH = TMP_DIR / "tmp_v7_bear_hysteresis_gate_README.md"
...
BEAR_GATE_REENTRY_CONFIRM_DAYS = 2
BEAR_GATE_FULL_CONFIRM_DAYS = 4
BEAR_GATE_COOLDOWN_DAYS = 3
...
def bear_gate_base_target_exposure(row):
    ...
    ma20_above_ma60 = bear_gate_int(row.get("ma20_above_ma60"), 0)
    slope60 = bear_gate_number(row.get("ma60_slope_20"), 0.0)
...
def apply_bear_gate_hysteresis(regime_frame):
    ...
    for _, row in out.iterrows():
        ...
        if base_target <= 0:
            ...
            if base_target >= 0.2 and bear_gate_number(row.get("breadth_above_ma20"), 0.0) > 0.35:
                reentry_streak += 1
            else:
                reentry_streak = 0
            if reentry_streak >= BEAR_GATE_REENTRY_CONFIRM_DAYS:
                current = 0.2
                cooldown = BEAR_GATE_COOLDOWN_DAYS
                reentry_streak = 0
            ...
            if current <= 0.2:
                ...
                if (
                    base_target >= 0.5
                    and bear_gate_number(row.get("market_drawdown_120"), 0.0) > -0.15
                    and bear_gate_number(row.get("breadth_above_ma20"), 0.0) > 0.40
                ):
                    reentry_streak += 1
                else:
                    reentry_streak = 0
                if cooldown <= 0 and reentry_streak >= BEAR_GATE_REENTRY_CONFIRM_DAYS:
                    current = 0.5
                    cooldown = BEAR_GATE_COOLDOWN_DAYS
                    reentry_streak = 0
                    full_streak = 0
                ...
                if base_target >= 1.0:
                    ...
                    full_streak = 0
                if cooldown <= 0 and full_streak >= BEAR_GATE_FULL_CONFIRM_DAYS:
                    current = 1.0
...
def load_v7_module(module_name: str, bear_gate_mode: str):
    ...
    try:
        ...
        if bear_gate_mode != "off":
            ...
            if bear_gate_mode == "defense_only":
...
def write_report(results: pd.DataFrame, yearly: pd.DataFrame, monthly: pd.DataFrame, reasons: pd.DataFrame) -> None:
    ...
    conclusion = "待人工复核"
    if "v7_bear_hysteresis_gate" in set(results["strategy"]) and "v7_baseline" in set(results["strategy"]):
        gate = results.set_index("strategy").loc["v7_bear_hysteresis_gate"]
    ...
    text = f"""<!doctype html>
<html lang="zh-CN"><head><meta charset="utf-8"><title>v7 熊市仓位状态机实验</title>
<style>
...
<h1>v7 熊市仓位状态机实验</h1>
<div class="note">
<b>策略：</b>非熊市走 v7 当前逻辑；v7 判断熊市后启用 tmp 的 hysteresis_fast_reentry 仓位状态机。<br>
<b>执行：</b>gate 版：target=0 清仓，target=20%/50% 限制熊市总敞口，target=100% 允许恢复普通买入；defense_only 版：只做清仓/降仓，不用 target=100% 放宽 v7 熊市阻断。熊市试探仍必须满足 bear_probe_market_ok 和 bear_probe_stock_ok。<br>
<b>反前视：</b>交易日 T 使用 T-1 signal_date 的 regime 和 target_exposure。<br>
...
def main() -> None:
    ...
    append_progress("读取 v6 输出基线。")
    v6_nav = pd.read_csv(V6_DIR / "output" / "nav_series.csv")
    v6_trades = pd.read_csv(V6_DIR / "output" / "trade_records.csv")
    rows.append(summarize_nav("v6_output", v6_nav, v6_trades))
    nav_map["v6_output"] = v6_nav
    trade_map["v6_output"] = v6_trades
    ...
    append_progress("运行 case=v7_bear_hysteresis_gate。")
    nav, trades, _, stats = run_case("v7_bear_hysteresis_gate", "gate", panel, market, end_date)
    rows.append(summarize_nav("v7_bear_hysteresis_gate", nav, trades, stats))
    nav_map["v7_bear_hysteresis_gate"] = nav
    trade_map["v7_bear_hysteresis_gate"] = trades

    append_progress("运行 case=v7_bear_hysteresis_defense_only。")
    nav, trades, _, stats = run_case("v7_bear_hysteresis_defense_only", "defense_only", panel, market, end_date)
    rows.append(summarize_nav("v7_bear_hysteresis_defense_only", nav, trades, stats))
    nav_map["v7_bear_hysteresis_defense_only"] = nav
    trade_map["v7_bear_hysteresis_defense_only"] = trades
