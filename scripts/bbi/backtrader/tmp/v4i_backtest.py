# v4i_backtest.py
# v4h 基础上加市场级别仓位控制（沪深300趋势过滤）
# 市场状态判断（用上周五数据，避免 look-ahead bias）：
#   牛市：close > MA60                    → 持仓 TOP_N=5
#   熊市：close < MA60 OR MA20 < MA60     → 持仓 TOP_N_BEAR=2
#   危机：close < MA60 AND MA20 < MA60 AND MA20 下降 → 空仓
# 个股止损保持 v4h 参数：ATR×3.0，硬止损6%，最短持仓10天
import sys
...
from v4i_config import (
    START_DATE, END_DATE,
    INIT_CASH, TOP_N, TOP_N_BEAR, TOP_N_CRISIS,
...
def load_index_data():
    """加载沪深300日线数据，计算 MA20/MA60，返回以 trade_date 为索引的 DataFrame"""
    ...
    # MA20 斜率：当前 MA20 vs 10天前 MA20
    df['ma20_slope'] = df['ma20'] - df['ma20'].shift(10)
...
def get_market_top_n(index_df, prev_date):
    """
    根据上周五（prev_date）的指数数据判断市场状态，返回本周允许的最大持仓数。
    危机（空仓）：close < MA60 AND MA20 < MA60 AND MA20 下降 → 0
    熊市（降仓）：close < MA60 OR MA20 < MA60               → TOP_N_BEAR=2
    牛市（满仓）：close > MA60                               → TOP_N=5
    """
    ...
    slope = float(row['ma20_slope']) if pd.notna(row['ma20_slope']) else 0

    # 危机：三重确认
    if close < ma60 and ma20 < ma60 and slope < 0:
        return TOP_N_CRISIS  # 0
...
def run_backtest(data, data_indexed, panel, name_map, index_df):
    ...
    for date in all_dates:
        ...
        if date in monday_set:
            ...
            market_state_log.append({'date': str(date)[:10], 'top_n': effective_top_n})
            ...
            if prev_date is not None and prev_date in panel_by_date:
                ...
                if effective_top_n == 0 or len(above) == 0:
                    new_picks = []
            ...
            # 如果市场进入危机，强制卖出超出限额的持仓
            if effective_top_n < len(cur_set):
                # 按浮盈从小到大排序，先卖亏损的
                ...
                to_sell_forced = set(sorted_holdings[:len(cur_set) - effective_top_n])
                new_set = new_set - to_sell_forced  # 确保这些被卖出
    ...
    print(f'Crisis weeks (空仓): {empty_weeks}')
    crisis_weeks = sum(1 for r in market_state_log if r['top_n'] == 0)
    bear_weeks   = sum(1 for r in market_state_log if r['top_n'] == 2)
    bull_weeks   = sum(1 for r in market_state_log if r['top_n'] == 5)
    print(f'Market state: 牛市{bull_weeks}周 / 熊市降仓{bear_weeks}周 / 危机空仓{crisis_weeks}周')
