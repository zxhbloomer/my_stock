# v4h_compare.py
# 对比 v4_plan_1（基准）和 v4h（收紧止损版）的回测结果
import sys
...
def main():
    print("=" * 60)
    print("  v4_plan_1 vs v4h（收紧止损）回测对比报告")
    print("  v4h: ATR倍数3.0（原4.5），硬止损6%（原8%），最短持仓10天")
    print("=" * 60)

    for p, name in [(V1_NAV, 'v4_plan_1 nav'), (V4H_NAV, 'v4h nav')]:
        if not p.exists():
            print(f"❌ 文件不存在: {p} ({name})")
            return

    m1, yr1 = calc_metrics(V1_NAV,  'v4_plan_1（基准）')
    m2, yr2 = calc_metrics(V4H_NAV, 'v4h（收紧止损）')

    print("\n【核心指标对比】")
    keys = ['累积净值', '年化收益', '最大回撤', '夏普比率', '卡玛比率', '最终资产(元)', '盈利(元)']
    print(f"{'指标':<16} {'v4_plan_1':>18} {'v4h':>18}")
    print("-" * 54)
    for k in keys:
    ...
    print(f"{'指标':<20} {'v4_plan_1':>16} {'v4h':>16}")
    print("-" * 54)
    for k in tkeys:
        v1 = str(t1.get(k, 'N/A'))
        v2 = str(t2.get(k, 'N/A'))
    ...
    print(f"{'年份':<8} {'v4_plan_1':>14} {'v4h':>14} {'差值':>10}")
    print("-" * 48)
    for yr in all_years:
    ...
    dd_better  = v2_dd > v1_dd
    sh_better  = v2_sh > v1_sh

    print(f"  年化收益: v4h {'↑' if ann_better else '↓'} {abs(v2_ann - v1_ann):.2f}pp vs v4_plan_1")
    print(f"  最大回撤: v4h {'改善' if dd_better else '恶化'} {abs(v2_dd - v1_dd):.2f}pp")
    print(f"  夏普比率: v4h {'↑' if sh_better else '↓'} {abs(v2_sh - v1_sh):.3f}")
    print("=" * 60)
