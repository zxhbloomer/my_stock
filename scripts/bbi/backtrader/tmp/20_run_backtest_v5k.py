# BBI v5k — 组合版：入场量能确认(0.8x) + 出场ADX门控
# 核心：把 v5h 的入场质量提升 和 v5g 的出场过滤 组合
#   入场：BBI金叉 + vol >= MA20 * 0.8（只过滤极度缩量，保留更多机会）
#   出场：BBI死叉需 ADX >= 20 才触发（震荡市忽略死叉）
#         + ATR间距过滤（粘合穿越忽略）
#   其余：硬止损/ATR移动止盈/MACD双死叉/筹码出场不变
import csv
...
VOL_MA_PERIOD       = 20
VOL_MULT_ENTRY      = 0.8   # 入场量能门槛：只过滤极度缩量
ADX_PERIOD          = 14
...
OUTPUT_SUBDIR       = TMP_OUTPUT_DIR / "v5k"
...
class BBIV5kStrategy(bt.Strategy):
    def __init__(self):
    ...
    def _entry_signal(self):
        ...
        vol_ok = (vol_ma_val > 0 and self.vol_line[0] >= vol_ma_val * VOL_MULT_ENTRY)
    ...
    def _macd_exit(self):
        ...
        return self.macd_line[0] < self.signal_line[0] and self.macd_line[0] < 0 and bbi_dec
...
def run_single_stock(args):
    ...
    try:
        ...
        cerebro = bt.Cerebro()
        cerebro.addstrategy(BBIV5kStrategy)
...
def main():
    ...
    print('[v5k] {} stocks | vol_entry={}x | ADX_exit={} | ATR_gap={}x'.format(
        len(args_list), VOL_MULT_ENTRY, ADX_THRESHOLD, ATR_GAP_MULT))
