# BBI v5a — exit optimization only (no new data)
# Hypothesis: extending MIN_HOLD_DAYS to 20 and ATR to 4.5x reduces premature exits
# Data shows: 1-20d hold has 0-9% win rate; 31-60d has 72%; 60d+ has 98%
# All entry logic identical to v2.4; only exit parameters changed
# 数据时序规则：仅使用 OHLCV+BBI，当日数据，无需 shift
import csv
...
ATR_MULTIPLIER_V5A = 4.5   # widened from v2.4's 3.5
MIN_HOLD_DAYS_V5A  = 20    # extended from v2.4's 10; data shows 21d+ win rate starts recovering
BBI_DECLINE_BARS   = 5     # BBI must decline for 5 bars (vs 3 in v2.4) to confirm dead cross
OUTPUT_SUBDIR      = OUTPUT_DIR / "v5a"
...
class BBIV5aStrategy(bt.Strategy):
    def __init__(self):
    ...
    def _exit_signal(self):
        ...
        # v5a: BBI must decline for 5 bars (not 3) to confirm sustained downtrend
    ...
    def _update_trail(self):
        ...
        if atr_val and atr_val > 0:
            new_stop = self.peak_close - ATR_MULTIPLIER_V5A * atr_val
    ...
    def _should_exit(self):
        ...
        if pos.size > 0:
            if (self.close_line[0] - pos.price) / pos.price <= -HARD_STOP_LOSS:
                return True
        if len(self) - self.buy_bar < MIN_HOLD_DAYS_V5A:
            return False
...
def run_single_stock(args):
    ...
    try:
        ...
        cerebro = bt.Cerebro()
        cerebro.addstrategy(BBIV5aStrategy)
...
def main():
    ...
    print(f'[v5a] {len(args_list)} stocks | ATR={ATR_MULTIPLIER_V5A}x | MIN_HOLD={MIN_HOLD_DAYS_V5A}d | BBI_DECLINE={BBI_DECLINE_BARS}bars')
