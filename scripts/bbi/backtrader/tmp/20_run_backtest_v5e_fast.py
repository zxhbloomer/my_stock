# BBI v5e_fast — 使用 stock_data_fast 目录，用于与 v5f 对比
# 与 v5e 逻辑完全相同，只改数据目录
import csv
...
FAST_STOCK_DATA_DIR = TMP_OUTPUT_DIR / "stock_data_fast"
OUTPUT_SUBDIR       = TMP_OUTPUT_DIR / "v5e_fast"
...
class BBIV5eStrategy(bt.Strategy):
    ...
    def _entry_signal(self):
        if len(self) < BBI_DECLINE_BARS + 1: return False
    ...
    def _chip_exit(self):
        ...
        return (wr == wr) and wr > CHIP_EXIT_THRESHOLD
    ...
    def _should_exit(self):
        ...
        if pos.size > 0 and (self.close_line[0] - pos.price) / pos.price <= -HARD_STOP_LOSS:
            return True
        if len(self) - self.buy_bar < MIN_HOLD_DAYS: return False
...
def run_single_stock(args):
    ...
    try:
        ...
        df = df.sort_index()
        if 'winner_rate' not in df.columns: return None, []
...
def main():
    ...
    parquet_files = sorted(FAST_STOCK_DATA_DIR.glob('*.parquet'))
    if not parquet_files:
        print('ERROR: No parquet files in {}'.format(FAST_STOCK_DATA_DIR)); return
    ...
    print('[v5e_fast] {} stocks | ATR={}x | MIN_HOLD={}d | chip_exit>{}'.format(
        len(args_list), ATR_MULTIPLIER, MIN_HOLD_DAYS, CHIP_EXIT_THRESHOLD))
