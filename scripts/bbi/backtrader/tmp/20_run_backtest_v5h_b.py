# BBI v5h_b — 入场成交量放大确认（在v5e基础上）
# 核心改动：BBI金叉入场时，要求当日成交量 >= N日均量 * VOL_MULT
#   原理：真实突破需要资金背书，缩量金叉多为假突破
#   A股特征：主力建仓必然放量，缩量上穿BBI大概率是洗盘后的假信号
#
# v5e baseline: calmar≈0.196, annual≈5.83%
# v5h adds: 入场要求 vol >= MA20_vol * 1.2
import csv
...
class BBIV5hStrategy(bt.Strategy):
    def __init__(self):
    ...
    def _entry_signal(self):
        ...
        # 成交量放大确认：当日量 >= 20日均量 * VOL_MULT
        vol_ma_val = self.vol_ma[0]
        vol_ok = (vol_ma_val > 0 and self.vol_line[0] >= vol_ma_val * VOL_MULT)
...
def run_single_stock(args):
    ...
    try:
        ...
        cerebro = bt.Cerebro()
        cerebro.addstrategy(BBIV5hStrategy)
...
def main():
    ...
    print('[v5h_b] {} stocks | ATR={}x | MIN_HOLD={}d | chip_exit>{} | vol_mult={}x'.format(
        len(args_list), ATR_MULTIPLIER, MIN_HOLD_DAYS, CHIP_EXIT_THRESHOLD, VOL_MULT))
