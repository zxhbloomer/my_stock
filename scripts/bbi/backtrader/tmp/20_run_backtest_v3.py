# BBI v3 strategy — tmp experiment
# Changes vs v2.3:
#   1. Dynamic ATR multiplier: 3.5 for first 10 bars, then 2.5 (give trend room to breathe)
#   2. ADX(14) > 20 entry filter (only enter confirmed trends, not choppy markets)
#   3. MACD death-cross exit requires ADX declining (reduces false exits in strong trends)
import csv
...
ADX_PERIOD        = 14
ADX_MIN_ENTRY     = 20    # only enter when trend is confirmed
ATR_EARLY_MULT    = 3.5   # wider stop for first N bars after entry
ATR_EARLY_BARS    = 10    # bars to use wider stop
OUTPUT_SUBDIR     = OUTPUT_DIR / "v3"
...
class BBIV3Strategy(bt.Strategy):
    def __init__(self):
        ...
        self.adx_ind     = bt.indicators.AverageDirectionalMovementIndex(self.data, period=ADX_PERIOD)
    ...
    def _entry_signal(self):
        if len(self) < max(ADX_PERIOD + 2, 4):
            return False
        ...
        adx_ok    = self.adx_ind[0] > ADX_MIN_ENTRY
        return cross_up and bbi_slope and macd_ok and adx_ok

    def _exit_signal(self):
        cross_down = self.close_line[-1] > self.bbi_line[-1] and self.close_line[0] < self.bbi_line[0]
        # MACD death cross: require ADX also declining to avoid false exits in strong trends
        adx_declining = len(self) >= 3 and self.adx_ind[0] < self.adx_ind[-2]
        macd_dead  = (self.macd_line[0] < self.signal_line[0]
                      and self.macd_line[0] < 0
                      and adx_declining)
        return cross_down or macd_dead

    def _current_atr_mult(self):
        held = len(self) - self.buy_bar
        return ATR_EARLY_MULT if held <= ATR_EARLY_BARS else ATR_MULTIPLIER
    ...
    def _update_trail(self):
        ...
        if atr_val and atr_val > 0:
            mult = self._current_atr_mult()
            new_stop = self.peak_close - mult * atr_val
...
def run_single_stock(args):
    ...
    try:
        ...
        cerebro = bt.Cerebro()
        cerebro.addstrategy(BBIV3Strategy)
...
def main():
    ...
    print(f'[v3] Running backtest on {len(args_list)} stocks with {N_WORKERS} workers...')
    print(f'     ADX filter: >{ADX_MIN_ENTRY}, ATR early mult: {ATR_EARLY_MULT} (first {ATR_EARLY_BARS} bars), then {ATR_MULTIPLIER}')
    ...
    print(f'  trades -> {OUTPUT_SUBDIR / "trades_detail.csv"}')
