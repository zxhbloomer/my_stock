# v4f_prepare_data.py
# 在 v4_plan_1/10_prepare_data.py 基础上额外保存 ma5、ma20、ma60 列
# 这三列是条件B趋势过滤所需：close > MA60 AND MA5 > MA20
import sys
...
def main():
    ...
    for i, row in df_stocks.iterrows():
        ...
        # 保留 ma5、ma20、ma60 供条件B过滤使用（ma10 不需要单独保存）
        # ma5、ma20、ma60 已在上面计算，直接保留
        ...
        # 删除 ma10（不需要单独列，BBI 已包含其信息）
        df = df.drop(columns=['ma10'])
