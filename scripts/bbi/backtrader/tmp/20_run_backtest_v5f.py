# BBI v5f — BBI死叉N日确认过滤（在v5e基础上）
# 核心改动：BBI死叉不再立即出场，而是等待N天确认
#   - 死叉发生后开始计时
#   - 若N天内收盘重新站回BBI上方 → 取消信号，继续持有
#   - 若N天后仍在BBI下方 → 确认出场
# ATR移动止盈和硬止损不受影响，仍然立即触发
# 假设：减少震荡市假死叉导致的频繁进出，提升持仓质量
#
# v5e baseline: ATR=4.5x, MIN_HOLD=20d, chip_exit>85, calmar≈0.094
# v5f adds: BBI_CONFIRM_DAYS=3 confirmation filter on BBI death cross
import csv
...
class BBIV5fStrategy(bt.Strategy):
    def __init__(self):
        ...
        # N日确认状态：记录死叉发生的bar编号，-1表示无待确认死叉
        self.bbi_cross_down_bar = -1
    ...
    def _update_bbi_confirm(self):
        """
        维护BBI死叉N日确认状态。
        - 若当前收盘 > BBI：价格回到BBI上方，取消待确认死叉
        - 若当前收盘 < BBI 且前日收盘 > 前日BBI：新死叉，开始计时
        - 返回True表示死叉已确认（计时满N天且仍在BBI下方）
        """
        close_now  = self.close_line[0]
        bbi_now    = self.bbi_line[0]
        close_prev = self.close_line[-1]
        bbi_prev   = self.bbi_line[-1]

        # 价格回到BBI上方 → 取消待确认死叉
        if close_now > bbi_now:
            self.bbi_cross_down_bar = -1
            return False

        # 新死叉发生（前日在上，今日在下）
        if close_prev > bbi_prev and close_now < bbi_now:
            self.bbi_cross_down_bar = len(self)

        # 检查是否已确认（死叉后经过N天，且仍在BBI下方）
        if self.bbi_cross_down_bar > 0:
            days_since_cross = len(self) - self.bbi_cross_down_bar
            if days_since_cross >= BBI_CONFIRM_DAYS:
                return True
    ...
    def _exit_signal(self):
        # BBI死叉：使用N日确认过滤（不再立即出场）
        bbi_confirmed_exit = self._update_bbi_confirm()
        return bbi_confirmed_exit or self._macd_exit() or self._chip_exit()
    ...
    def _update_trail(self):
        ...
        if atr_val and atr_val > 0:
            new_stop = self.peak_close - ATR_MULTIPLIER_V5F * atr_val
    ...
    def _should_exit(self):
        ...
        if pos.size > 0:
            if (self.close_line[0] - pos.price) / pos.price <= -HARD_STOP_LOSS:
                return True  # 硬止损立即触发，不受确认期影响
        held = len(self) - self.buy_bar
        if held < MIN_HOLD_DAYS_V5F:
            # 保护期内：只更新BBI确认计时（让死叉计时在保护期内就开始积累）
            # _exit_signal 不调用，避免重复更新状态
            self._update_bbi_confirm()
            return False
        # ATR移动止盈独立于BBI确认，立即触发
...
def run_single_stock(args):
    ...
    try:
        ...
        cerebro = bt.Cerebro()
        cerebro.addstrategy(BBIV5fStrategy)
...
def main():
    ...
    print('[v5f] {} stocks | ATR={}x | MIN_HOLD={}d | chip_exit>{} | BBI_CONFIRM={}d'.format(
        len(args_list), ATR_MULTIPLIER_V5F, MIN_HOLD_DAYS_V5F, CHIP_EXIT_THRESHOLD, BBI_CONFIRM_DAYS))
