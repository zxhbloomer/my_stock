...
OUTPUT_DIR = TMP_DIR / "tmp_v6_2018_bear_defensive_evolution_output"
REPORT_PATH = OUTPUT_DIR / "report.html"
RESULTS_PATH = OUTPUT_DIR / "results.csv"
README_PATH = TMP_DIR / "tmp_v6_2018_bear_defensive_evolution_README.md"
...
CASES = [
    {"name": "base_20_monthly", "max_holdings": 20, "max_per_industry": None, "industry_neutral": False, "quality_trap": False},
    {"name": "industry_cap_30_monthly", "max_holdings": 30, "max_per_industry": 3, "industry_neutral": False, "quality_trap": False},
    {"name": "industry_neutral_30_monthly", "max_holdings": 30, "max_per_industry": 3, "industry_neutral": True, "quality_trap": False},
    {"name": "quality_trap_30_monthly", "max_holdings": 30, "max_per_industry": 3, "industry_neutral": True, "quality_trap": True},
]
...
def load_support_data(db_url: str, schema: str, start_date: str, end_date: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
...
def render_report(compare: pd.DataFrame, case_results: pd.DataFrame, yearly: pd.DataFrame, monthly: pd.DataFrame, notes: str):
    ...
    text = f"""<!doctype html>
<html lang="zh-CN"><head><meta charset="utf-8"><title>2018 独立熊市防御策略进化</title>
<style>
...
<h1>2018 独立熊市防御策略进化</h1>
<div class="note">本轮只优化独立熊市策略，不接入 v6。行业数据使用 stock_basic.industry，未使用空的申万成员表。</div>
...
def main():
    ...
    README_PATH.write_text("# tmp_v6 2018 独立熊市防御策略进化进度\n\n", encoding="utf-8")
    ...
    append_progress("完成 daily_basic/fina_indicator/stock_basic 行业字段加载。")

    base_panel = build_base_panel(panel, market, daily_basic, fina, stock_basic)
    append_progress("完成基础面板构建。")

    case_rows = []
    nav_map = {"v4": pd.read_csv(BACKTRADER_DIR / "v4" / "output" / "nav_series.csv"), "v5": pd.read_csv(BACKTRADER_DIR / "v5" / "output" / "nav_series.csv"), "v6": v6_nav}
    trade_map = {"v4": pd.read_csv(BACKTRADER_DIR / "v4" / "output" / "trade_records.csv"), "v5": pd.read_csv(BACKTRADER_DIR / "v5" / "output" / "trade_records.csv"), "v6": v6_trades}
    best_case_name = None
    ...
    compare_rows = []
    for name in ["v4", "v5", "v6", best_case_name]:
    ...
    monthly = period_return_table({k: slice_2018(v) for k, v in nav_map.items() if k in ["v4", "v5", "v6", best_case_name]}, "M")
    ...
    notes = (
        "策略专家意见：优先测试低波红利质量、行业约束、行业内标准化，避免 2018 单年过拟合。\n"
        "代码/数据说明：行业使用 stock_basic.industry；申万成员表当前为空，未纳入。\n"
        ...
        f"建议：{merge_advice}。\n"
        "下一步：如果最佳变体仍未优于 v6，则应继续优化红利事件口径、轻度指数风控和仓位暴露，而不是扩大参数搜索。"
    )
