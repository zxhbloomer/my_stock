...
OUTPUT_DIR = TMP_DIR / "tmp_v7_pure_bull_winner_add_output"
REPORT_PATH = OUTPUT_DIR / "report.html"
README_PATH = TMP_DIR / "tmp_v7_pure_bull_winner_add_README.md"
...
CASES = [
    ...
    },
    {
        "case": "纯牛市小额最后加仓",
        "mode": "pure_bull_winner_add",
        "steps": (80_000.0, 60_000.0, 40_000.0, 20_000.0, 50_000.0),
...
def is_extra_step(step_index):
    return int(step_index) == 4


def calc_total_exposure_limit(case, nav, market_regime_name, is_extra_winner_add):
    if (
        case.get("mode") == "baseline"
        or market_regime_name != "bull"
        or not bool(is_extra_winner_add)
    ):
...
def can_use_extra_winner_add(case, step_index, market_regime_name, current_nav, peak_nav):
    if case.get("mode") == "baseline":
        return False
    if not is_extra_step(step_index):
        return False
...
def can_use_normal_v7_add(case, step_index, market_regime_name, current_nav, peak_nav):
    _ = case, market_regime_name, current_nav, peak_nav
    return 0 < int(step_index) < 4


def build_patched_v7_source(case):
    _ = case
    ...
    source = replace_once(
        ...
        '        "weak_lowvol_mom_candidate_blocks": 0,\n'
        '        "pure_bull_case": PURE_BULL_CASE.get("case", "baseline"),\n'
    ...
    source = replace_once(
        ...
        "                        is_extra_winner_add = __is_extra_winner_add(step_index_before_add)\n"
        ...
        "                                continue\n"
        "                        available_exposure = __total_exposure_limit(current_nav, market_regime_name, is_extra_winner_add) - calc_total_exposure(holdings)\n"
        "                        target_amount = min(target_amount, available_exposure)\n",
    )
    source = replace_once(
        source,
        ...
        "                        if bought and is_extra_winner_add:\n"
        "                            stats[\"pure_bull_extra_add_fills\"] += 1\n"
        "                            stats[\"pure_bull_extra_add_amount\"] += float(target_amount)\n"
        "                            holdings[code][\"pure_bull_extra_amount\"] = float(holdings[code].get(\"pure_bull_extra_amount\", 0.0)) + float(target_amount)\n"
        "                        if bought:\n",
    )
    source = replace_once(
        source,
...
def load_v7_backtest_module(module_name, case):
    ...
    try:
        source = build_patched_v7_source(case)
        ...
        def _total_exposure_limit(current_nav, market_regime_name, is_extra_winner_add):
            return calc_total_exposure_limit(
                module.PURE_BULL_CASE,
                current_nav,
                market_regime_name,
                is_extra_winner_add,
            )
        ...
        def _can_use_extra_winner_add(step_index, market_regime_name, current_nav):
            return can_use_extra_winner_add(
                ...
                market_regime_name,
                current_nav,
                module.__pure_bull_peak_nav,
            )

        def _is_extra_winner_add(step_index):
            return is_extra_step(step_index)

        def _record_pure_bull_nav(nav):
            module.__pure_bull_peak_nav = max(float(module.__pure_bull_peak_nav), float(nav))
        ...
        module.__is_extra_winner_add = _is_extra_winner_add
        module.__record_pure_bull_nav = _record_pure_bull_nav
        return module
    finally:
...
def extra_add_sell_attribution(trades):
    ...
    for _, row in trades.iterrows():
        ...
        if row["action"] == "buy":
            ...
            pos["amount"] += amount
            if row["reason"] == "pure_bull_extra_add":
                pos["extra"] += amount
...
def run_case(case, panel, market):
    module = load_v7_backtest_module("tmp_v7_pure_bull_" + case["case"], case)
    ...
    if not trades.empty and "reason" in trades.columns:
        extra_buys = trades[trades["reason"] == "pure_bull_extra_add"]
        stats["pure_bull_extra_add_amount"] = float(extra_buys["amount"].sum())
...
def best_candidate(all_stats):
    candidates = [s for s in all_stats if s.get("mode") == "pure_bull_winner_add"]
    if not candidates:
        return None
...
def build_report(all_stats, nav_by_case):
    ...
    for stats in all_stats:
        summary_rows.append({
            ...
            "额外加仓次数": stats.get("pure_bull_extra_add_fills", "-"),
            "额外加仓金额": f"{float(stats.get('pure_bull_extra_add_amount', 0.0)):.0f}",
            "额外加仓粗估PnL": f"{float(stats.get('extra_add_pnl_est', 0.0)):.0f}",
    ...
    if best and v7:
        ...
        if improves and dd_ok:
            advice = "建议进入合入候选"
            reason = "最佳实验收益超过 v7，最大回撤恶化不超过 3 个百分点；正式合入前还需做年度和状态稳健性复核。"
    best_text = "无"
    if best:
...
<body>
<h1>v7 纯牛市赢家加仓实验报告</h1>
<div class="note"><b>合并建议：{html.escape(advice)}</b><br>{html.escape(reason)}<br>最佳实验：{html.escape(best_text)}</div>
<p class="small">本轮最多仍持有5只，只在信号日为 bull 时触发第5次额外赢家加仓；neutral/bear 不新增额外买入，但会继续持有 bull 期间已经买入的仓位。正式 v4/v5/v6/v7 文件未修改。</p>
{table_html(summary_df, "核心指标对比")}
{table_html(yearly_df, "年度收益对比（%）")}
{table_html(monthly_df, "最近36个月月度收益对比（%）")}
<h2>下一步建议</h2>
<ol>
<li>最佳结果可作为合入候选，但先做分年度、分 market_regime 的稳健性复核。</li>
<li>额外加仓粗估 PnL 只用于方向观察，不能单独证明因果。</li>
<li>若后续稳健性弱于 v7，回到失败买入过滤，而不是继续放大单票。</li>
</ol>
</body>
</html>"""
...
def main():
    ...
    for case in CASES:
        ...
        if not trades.empty and "reason" in trades.columns:
            extra_audit = trades[trades["reason"].eq("pure_bull_extra_add")].copy()
            if not extra_audit.empty and not rebalance_log.empty:
                extra_audit = extra_audit.merge(
                    rebalance_log[["date", "signal_date", "market_regime"]],
                    on="date",
                    how="left",
                )
            extra_audit.to_csv(OUTPUT_DIR / f"{case['case']}_extra_add_audit.csv", index=False, encoding="utf-8-sig")
        ...
        append_progress(
            ...
            f"extra_adds={stats.get('pure_bull_extra_add_fills', 0)}。"
        )

    build_report(all_stats, nav_by_case)
    append_progress(f"生成 HTML 报表：{REPORT_PATH}")
    append_progress("设计 review：专家角色确认本轮只改 bull 的第5次赢家加仓，不增加持股数。")
    append_progress("开发 review：检查项包括 neutral/bear 禁止触发额外买入、普通 v7 仓位上限不变、严格回撤门控、额外加仓统计。")
    append_progress("最终 review：bull 额外仓位会被带入后续 neutral/bear，因此本轮结论是买入动作 bull-only，不是后续路径完全等同 v7。")
