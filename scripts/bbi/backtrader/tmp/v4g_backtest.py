# v4g_backtest.py
# v4f 进化版：条件B改为 close > MA60 AND MA20 > MA60
# 原 v4f 用 MA5>MA20，在熊市底部持续为 False，导致错过整个反弹
# v4g 用 MA20>MA60（中期在长期上方），信号更稳定，减少假信号
import sys
...
def apply_trend_filter_g(above, prev_date, data):
    """
    条件G过滤：close > MA60 AND MA20 > MA60
    比 v4f 的 MA5>MA20 更稳定：MA20>MA60 是中期趋势确认，
    在熊市底部不会立即翻负，减少假信号和过度空仓。
    """
    passed = []
    ...
    for date in all_dates:
        ...
        if date in monday_set:
            if prev_date is not None and prev_date in panel_by_date:
                ...
                # 条件G过滤：close > MA60 AND MA20 > MA60
                trend_ok = apply_trend_filter_g(above, prev_date, data)
