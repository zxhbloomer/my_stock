# BBI v4a — volume confirmation on entry
# Hypothesis: adding volume > 1.5x 20-day avg as entry filter reduces false breakouts
# (hard stops are 22.9% of trades — most are false breakouts with no volume support)
# All other logic identical to v2.4
# 数据时序规则：vol 是当日盘中数据，可直接使用。若后续加入 moneyflow/cyq_perf，必须在 prepare_data 中 shift(1)。
import csv
...
VOL_MA_PERIOD   = 20
VOL_MULTIPLIER  = 1.5   # entry requires vol > 1.5x 20-day avg
OUTPUT_SUBDIR   = OUTPUT_DIR / "v4a"
...
class BBIV4aStrategy(bt.Strategy):
    def __init__(self):
    ...
    def _entry_signal(self):
        if len(self) < max(VOL_MA_PERIOD + 1, 4):
            return False
        ...
        vol_ok      = self.vol_line[0] > self.vol_ma[0] * VOL_MULTIPLIER
        return cross_up and bbi_slope and macd_ok and vol_ok
...
def run_single_stock(args):
    ...
    try:
        ...
        cerebro = bt.Cerebro()
        cerebro.addstrategy(BBIV4aStrategy)
...
def main():
    ...
    print(f'[v4a] {len(args_list)} stocks, vol filter: >{VOL_MULTIPLIER}x {VOL_MA_PERIOD}d avg')
