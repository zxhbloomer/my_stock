"""
接口：stk_auction_c，可以通过数据工具调试和查看数据
描述：股票收盘15:00集合竞价数据，每天盘后更新
限量：单次请求最大返回10000行数据，可根据日期循环
权限：开通了股票分钟权限后可获得本接口权限
接口文档: https://tushare.pro/document/2?doc_id=354
本地文档: docs/tushare/tushare.pro/document/2a46b.html

输入参数：ts_code(str,N,股票代码), trade_date(str,N,交易日期),
          start_date(str,N,开始日期), end_date(str,N,结束日期)
输出字段：ts_code,trade_date,close,open,high,low,vol,amount,vwap

同步策略：按交易日增量（ts_code+trade_date 为主键，upsert）
表名：068_stk_auction_c
迁移说明：tushare schema 中无此表，无需迁移
用法: python 068_stk_auction_c.py [--start YYYYMMDD] [--end YYYYMMDD]
"""
...
FLOAT_COLS = ["close","open","high","low","vol","amount","vwap"]
...
def main():
    ...
    for i, d in enumerate(dates, 1):
        try:
            df = pro.stk_auction_c(trade_date=d, fields=FIELDS)
