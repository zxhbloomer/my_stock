"""
接口：express，可以通过数据工具调试和查看数据
描述：获取上市公司业绩快报
限量：用户需要至少2000积分才可以调取；当前接口只能按单只股票获取其历史数据，
      如需获取某一季度全部上市公司数据，请使用express_vip接口（需5000积分）
权限：2000积分以上可以调取
接口文档: https://tushare.pro/document/2?doc_id=46
本地文档: docs/tushare/tushare.pro/document/2bdee.html

输入参数：ts_code(str,Y,股票代码), ann_date(str,N,公告日期),
          start_date(str,N,公告开始日期), end_date(str,N,公告结束日期),
          period(str,N,报告期)
输出字段：ts_code,ann_date,end_date,revenue,operate_profit,total_profit,n_income,
...
表名：040_express
迁移说明：tushare.express 有数据，字段完全一致，可直接迁移
用法: python 040_express.py [--start YYYYMMDD] [--end YYYYMMDD]
"""
...
FLOAT_COLS = ["revenue","operate_profit","total_profit","n_income","total_assets",
