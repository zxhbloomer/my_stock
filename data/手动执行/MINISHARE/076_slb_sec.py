"""
接口：slb_sec，可以通过数据工具调试和查看数据
描述：转融通转融券交易汇总（注：该接口已停止更新）
限量：单次最大可以提取5000行数据，可循环获取所有历史
权限：2000积分每分钟请求200次，5000积分500次请求
接口文档: https://tushare.pro/document/2?doc_id=332
本地文档: docs/tushare/tushare.pro/document/2a7a1.html

输入参数：trade_date(str,N,交易日期), ts_code(str,N,股票代码),
          start_date(str,N,开始日期), end_date(str,N,结束日期)
输出字段：trade_date,ts_code,name,ope_inv,lent_qnt,cls_inv,end_bal

同步策略：按交易日增量（trade_date+ts_code 为主键，upsert）
          注意：该接口已停止更新，历史数据仍可获取
表名：076_slb_sec
迁移说明：tushare schema 中无此表，无需迁移
用法: python 076_slb_sec.py [--start YYYYMMDD] [--end YYYYMMDD]
"""
...
TABLE         = "076_slb_sec"
DEFAULT_START = "20130201"  # 转融通2013年2月开始
LOOKBACK_DAYS = 3
...
FLOAT_COLS = ["ope_inv","lent_qnt","cls_inv","end_bal"]
...
def main():
    ...
    for i, d in enumerate(dates, 1):
        try:
            df = pro.slb_sec(trade_date=d, fields=FIELDS)
