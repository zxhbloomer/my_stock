"""
接口：moneyflow_ind_ths，可以通过数据工具调试和查看数据
描述：获取同花顺行业资金流向，每日盘后更新
限量：单次最大可调取5000条数据，可以根据日期和代码循环提取全部数据
权限：6000积分可以调取
接口文档: https://tushare.pro/document/2?doc_id=351
本地文档: docs/tushare/tushare.pro/document/2a56d.html

输入参数：ts_code(str,N,代码), trade_date(str,N,交易日期),
          start_date(str,N,开始日期), end_date(str,N,结束日期)
输出字段：trade_date,ts_code,industry,lead_stock,close,pct_change,company_num,
          pct_change_stock,close_price,net_buy_amount,net_sell_amount,net_amount

同步策略：按交易日增量（trade_date+ts_code 为主键，upsert）
表名：084_moneyflow_ind_ths
迁移说明：新接口脚本，目标表不存在，无需迁移
用法: python 084_moneyflow_ind_ths.py [--start YYYYMMDD] [--end YYYYMMDD]
"""
...
FLOAT_COLS = ["close","pct_change","pct_change_stock","close_price",
...
def main():
    ...
    for i, d in enumerate(dates, 1):
        ...
        try:
            df = pro.moneyflow_ind_ths(trade_date=d, fields=FIELDS)
