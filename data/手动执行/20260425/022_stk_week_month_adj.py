"""
接口：stk_week_month_adj，可以通过数据工具调试和查看数据
描述：股票周/月线行情(复权--每日更新)
限量：单次最大6000，可使用交易日期循环提取，总量不限制
权限：用户需要至少2000积分才可以调取
接口文档: https://tushare.pro/document/2?doc_id=230
本地文档: docs/tushare/tushare.pro/document/294ac.html

输入参数：ts_code(str,N,TS代码), trade_date(str,N,交易日期),
          start_date(str,N,开始交易日期), end_date(str,N,结束交易日期),
          freq(str,Y,频率 week周/month月)
输出字段：ts_code,trade_date,end_date,freq,open,high,low,close,pre_close,
...
表名：022_stk_week_month_adj
迁移说明：新接口脚本，目标表不存在，无需迁移
用法: python 022_stk_week_month_adj.py [--start YYYYMMDD] [--end YYYYMMDD]
"""
...
FLOAT_COLS = ["open","high","low","close","pre_close","open_qfq","high_qfq",
              "low_qfq","close_qfq","open_hfq","high_hfq","low_hfq","close_hfq",
              "vol","amount","change","pct_chg"]
    ...
    for i, d in enumerate(dates, 1):
        ...
        try:
            ...
            if d in month_dates:
                freqs.append("month")
            for freq in freqs:
                df = pro.stk_week_month_adj(trade_date=d, freq=freq, fields=FIELDS)
