"""
接口：moneyflow_mkt_dc，可以通过数据工具调试和查看数据
描述：获取东方财富大盘资金流向数据，每日盘后更新
限量：单次最大3000条，可根据日期或日期区间循环获取
权限：120积分可试用，6000积分可正式调取
接口文档: https://tushare.pro/document/2?doc_id=353
本地文档: docs/tushare/tushare.pro/document/27ea5.html

输入参数：trade_date(str,N,交易日期), start_date(str,N,开始日期), end_date(str,N,结束日期)
输出字段：trade_date,close_sh,pct_change_sh,close_sz,pct_change_sz,net_amount,net_amount_rate,
...
表名：086_moneyflow_mkt_dc
迁移说明：新接口脚本，目标表不存在，无需迁移
用法: python 086_moneyflow_mkt_dc.py [--start YYYYMMDD] [--end YYYYMMDD]
"""
...
FLOAT_COLS = ["close_sh","pct_change_sh","close_sz","pct_change_sz","net_amount",
...
def main():
    ...
    for i, d in enumerate(dates, 1):
        ...
        try:
            df = pro.moneyflow_mkt_dc(trade_date=d, fields=FIELDS)
