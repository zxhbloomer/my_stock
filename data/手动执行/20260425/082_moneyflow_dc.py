"""
接口：moneyflow_dc，可以通过数据工具调试和查看数据
描述：获取东方财富个股资金流向数据，每日盘后更新，数据开始于20230911
限量：单次最大获取6000条数据，可根据日期或股票代码循环提取数据
权限：用户需要至少5000积分才可以调取
接口文档: https://tushare.pro/document/2?doc_id=341
本地文档: docs/tushare/tushare.pro/document/20c4d.html

输入参数：ts_code(str,N,股票代码), trade_date(str,N,交易日期),
          start_date(str,N,开始日期), end_date(str,N,结束日期)
输出字段：trade_date,ts_code,name,pct_change,close,net_amount,net_amount_rate,
...
用法: python 082_moneyflow_dc.py [--start YYYYMMDD] [--end YYYYMMDD]
"""
...
TABLE         = "082_moneyflow_dc"
DEFAULT_START = "20230911"
...
def main():
    ...
    for i, d in enumerate(dates, 1):
        ...
        try:
            df = pro.moneyflow_dc(trade_date=d, fields=FIELDS)
