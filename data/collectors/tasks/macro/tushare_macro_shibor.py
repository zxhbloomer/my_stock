...
@task_register()
class TushareMacroShiborTask(TushareTask):
    """获取上海银行同业拆借利率SHIBOR数据"""
    ...
    # 2. TushareTask 特有属性
    api_name = "shibor"
    # Tushare shibor 接口实际返回的字段 (根据Tushare文档)
    fields = ["date", "on", "1w", "2w", "1m", "3m", "6m", "9m", "1y"]

    # 3. 列名映射 (API字段名与数据库列名不完全一致，进行映射)
    column_mapping = {"1y": "12m"}  # 将1y映射为12m，保持与其他利率表命名一致
    ...
    schema_def = {
        ...
        "9m": {"type": "NUMERIC(10,4)"},  # 9个月拆借利率
        "12m": {"type": "NUMERIC(10,4)"},  # 12个月拆借利率 (映射后的名称)
        # update_time 会自动添加
    }

    # 6. 自定义索引
    indexes = [
        {"name": "idx_shibor_date", "columns": "date"},
        {"name": "idx_shibor_update_time", "columns": "update_time"},
    ]
